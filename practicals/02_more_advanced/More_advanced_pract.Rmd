...
This is the second practical, we will dive into more advanced matters: model selection, subject specific parameters, model diagnostics, and model fit. Note that we cover a lot of ground here, most likely too much to finish within this practical. All parts are modular, so feel free to work on the aspects that are most relevant to you. 

We continue with the same dataset: the open access dataset from Rowland and Wenzel (2020), using the subset of the affective experiences *happy*, *relaxed*, *anxious*, and *depressed*, measured six times a day for 40 days in total. 

The answers to the practicals are available in the `*_answers.html` files.

In all practicals in this workshop, we make use of the `mHMMbayes` package, version 1.1.0. We again load the package:
...
source('./helpers.R') # Some helper functions
```
...
We first obtain the subject specific emission distribution parameters from the model output, and save the subject specific parameters in a new object. When focusing on the emission distribution parameters, the function `obtain_emiss()` can be used to obtain the subject specific emission distribution parameters, by setting the input argument `level="subject"`. Note that the default of `level` is `"group"`, so when left unspecified, the function `obtain_emiss()` will return the group level emission distribution parameters, as we have seen in the first practical. 
...
Again, to get an overview of how much the emission distribution parameters (e.g., the variable and state specific means) vary over subjects, it is helpful to plot the subject specific emission distribution parameters, for example by adding the subject specific parameters as dots to a group level barplot of the emission means. This can be done using the *ggmHMM* function `plot_emiss()`.
...
```{r, include = params$answers}
# using ggmHMM package
plot_emiss(out_2st_emotion, 
           subject_effects = TRUE, 
           line = FALSE, 
           subject = 1:50) # Only plot subset for cleaner figure
```

---

In addition, it is important to check that the states represent the same construct over subjects. In the current dataset, that would mean that for each subject, state 1 represents a 'positive mood' state, and state 2 represents a 'negative mood' state. For example, for the indicator *happy*, that would translate to the mean of state 1 being higher compared to state 2. Visually, this can be checked by connecting the subject specific means over the states within one indicator by lines and assessing whether many lines are crossing (they should not!).

---

**Exercise 3**
Plot (a subset of) the subject specific emission distribution means connecting the subject specific means, and visually investigate whether the state 1 is a relatively positive state and state 2 a relatively negative state over subjects. Hint: this can be done by specifying `line = TRUE` in `plot_emiss()`.
...
plot_emiss(out_2st_emotion, 
           subject_effects = TRUE, 
           line = TRUE, 
           subject = 1:50) # using ggmHMM function
```
...
First, we need to obtain the subject specific parameters for the emissions and the transition probabilities. Analogously to the emission parameters, the function `obtain_gamma()` can also be used to obtain the subject specific transition probabilities, by setting the input argument `level="subject"`. Again, the default of `level` is `"group"`, so when left unspecified, the function `obtain_gamma()` will return the group level transition probabilities. 
...
* With the helper function `PlotTrans()`, a bar plot may be plotted showing the group level effects, with points for the subject-specific effects (or a subset thereof). It uses the following arguments:
...
PlotTrans(out_2st_emotion, 
          subject = 1:50) 

# Transition probabilities of first 10 subjects using ggmHMM function
plot_gamma(out_2st_emotion, 
           level = "subject", 
           subject = 1:10, 
           ncol_facet = 5) # plot with facets
```
...
One of the challenges when using hidden Markov models is that in most cases we do not know how many states we should choose and therefore need to learn the number of states from the data. Here, we assume that only a model with 2, 3, or 4 states is plausible and will perform model selection between those models. However, the model selection discussed here naturally extends to any sequence of candidate models.

As checking model fit is discussed at a later point in the practical, we will investigate the different models based on the state composition and the AIC. However, it should be noted that convergence and model fit are important characteristics that should be considered when choosing a model to interpret. The 2-state model was fitted in the first practical, and saved in the object `out_2st_emotion`. 
...
**Exercise 10**
Obtain the AIC values of the 2-, 3-, and 4-state model using the `print()` function on the mHMM objects `out_2st_emotion`, `out_3st_emotion`, `out_4st_emotion`, and compare the models. It can be helpful to visualize the AIC values over the states:
...
Determining the number of states is often not a trivial task and typically involves several considerations (including model fit and fit indices like the AIC) that themselves often depend on the research question at hand.
...
Evidence for non convergence can be checked in multiple ways. A first way is, using trace plots which visualize the parameter estimates over the iterations of the MCMC sampler. The chains should not show any trend and good mixing. Furthermore, we should have no label switching (or at least very little) in the subject-specific parameters (see the section 'label switching' below).
...
We should also inspect convergence numerically, with the potential scale reduction factor $\hat{R}$. $\hat{R}$ evaluates equality of means of the different chains - the degree to which variance (of the means) between chains exceeds what one would expect if the chains were identically distributed. A value of R-hat below 1.2 is typically used as a rule of thumb to indicate convergence. 
...
A concept related to convergence is that of label switching. In a multilevel HMM, it does not matter for the model what the ordering of the states is. For example, a model where State 1 is the 'positive' state (e.g. with high "happy" and low "depressed"), and state 2 the 'negative' state, or whether it is the other way around. In the iterative MCMC process, the model may switch the labels of the states over iterations, and therefore the interpretation of the states. This is called label switching. In the mHMM, label switching happens at the subject level, but consequently also affects estimates of the group-level effects. It is important to check to what extent this happens.

To check for label switching, inspect trace plots for the subject-level parameters, similar to those from last exercise. Ideally one displayes trace plots from multiple states in a single plot to better see whether the states switch labels. This can be done with the helper function `plot_label_switching()`.
...
**Exercise 17**
Use the script below to simulate 100 datasets based on the model parameters of the 2-state multilevel HMM. 
...
We have provided you with a helper function, `PlotResid()`, to plot the pseudoresiduals. Visually inspecting the predictions vs. the respective empirical data and the residuals is useful because it allows us not only to judge how well the model predicts, but also in which way predictions fail. This, in turn, may allow us to improve the model or at least draw the correct conclusions from the current model.

`PlotResid()` takes the following arguments:
