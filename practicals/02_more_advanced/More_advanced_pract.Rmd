...
**Exercise 6**
Interpret the transition probabilities. To what extent would you say that subjects differ in their transition probabilities? Are subjects generally more likely to stay in the same state, or switch states? What does this mean, if we take the substantive interpretation of the states into account?
...
**Exercise 7**
To save time we have already pre-run a 3-state and 4-state model. Use the function `readRDS()` to load the fitted models from the files `out_3st_emotion.rds` and `out_4st_emotion.rds`, and save them in the objects `out_3st_emotion` and `out_4st_emotion`.
...
**Exercise 9**
Use the helper function `PlotEmiss()` or the function `plot_emiss()` from the *ggmHMM* package to visualize the emission distributions of the three-state and four-state model. Do not plot subject-specific effects. Does it seem to make sense to add extra state(s)?
...
**Exercise 12**
To save time, we will provide you with an adittional chain, run with the starting values below. Use the function `readRDS()` to load the fitted model from the file `out_2st_emotion_b.rds`, and save it in the object `out_2st_emotion_b`.
...
For the 3-state model, use `plot_trace()` to create a trace plot for the subject-specific means of the variable "relaxed" for subject number 9. Do the same for the variable "depressed". What do you notice?

```{r, include=params$answers}
# for "relaxed"
plot_trace(model = out_3st_emotion,
           component = "emiss",
           level = "subject",
           vrb = "relaxed",
           subject = 9)
...
### There seem to be multiple switches, for example a switch around iteration 30 between state 2 and 3. It switches from state 2 being the "medium" state, with medium values for "relaxed" and "depressed", and state 3 being the "bad" state, with low values for "relaxed" and high values for "depressed", to state 2 being the "bad" state, and state 3 being the "medium" state. It switches back at around iteration 75 and then again at around iteration 325
```
