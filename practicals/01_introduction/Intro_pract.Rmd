...
In this section, you will fit a 2-state multilevel hidden Markov model. Note that in an full empirical analysis we will have to do model selection between models with different number of states. However, here we first get to know the model by assuming that we know the number of states. We will look at model selection in the second practical.

To fit an mHMM with the `mHMMbayes` package, we need a `data.frame` including only the variables we would like to model, plus the subject ID as the first column.
...
We will start with the specification of starting values of the emission distribution (i.e. the state-specific means and standard deviation). If we assume a 2-state model, for the type of variables we model here, it is plausible to assume that we will have a 'positive' state (high positively valenced emotions, low negatively valenced emotions) and a 'negative' state (the other way around). Define the means of the variable and state specific emission distributions to align with this idea. **Do not use the same starting values for means for a dependent variable across states!** A natural approach would be to base starting values on the characteristics of the empirical data; we can look at the observed range for each variable, and split the data in 2 in the middle of this range, creating 2 subsets. Within each subset, we can then evaluate the median and use these as starting values. However, make sure that the starting values are not too close to the boundary of the observed range, and that the starting values for each state are not too far apart in relation to the standard deviation.

Regarding the standard deviations of the emission distribution, define them such that the emission distribution for state 1 and 2 together cover the entire range of observations (recall that the response scale was a slider [0 to 100]). For example, given our data, it would not make sense to use a standard deviation of 3 (as 95% of the mass of the distribution will only span mean+/- 2 * 3 values out of 100); a standard deviation of 10 or 15 would make more sense.
...
**B** Now, specify the obtained values as starting values. First, set up the model input arguments: `m` for the number of states the model should infer, `n_dep` for the number of dependent variables used for fitting the model, and then the starting values for the emission distribution, which is provided in the object `start_emiss`. `start_emiss` should be a list with `n_dep` elements. Each element within the list should be a matrix with 2 columns and `m` rows. The means are contained in the first column of this matrix, and the standard deviations are contained in the second column of the matrix.
...
Make sure that the starting values are sensible. That is, for gamma, usually the elements contained on the diagonal (i.e., the self-transitions) are larger than the off-diagonal values to indicate that from one time point to another, subjects are more likely to remain in the same state than to switch to a new one, when time points are only a few hours apart as in most EMA studies.
...
Since the *mHMMbayes* package uses Bayesian estimation, we need to set up priors for all model parameters. When available, priors allow us to incorporate existing knowledge and can be specified manually using `prior_gamma()` for the transition probability matrix, and `prior_emiss_cont()` for the emission distributions for continuous data. However, here we will use the default priors specified in those functions.
...
* emiss_mu0: the parameters specifying the means for the Normal emission distribution. In our example, the means should align with the hypothesized structure of a 'positive emotion' and a 'negative emotion' state. Similar to the starting value, do not specify the same means for a given variable across states. It should be a list of *n_dep* matrices (one for each observed variable), each with 1 row and *m* columns (one for each state). The first matrix for example, specifies the prior for the means of 'happy' in state 1 and state 2.
...
Here, we show an example of such a plot for a weakly informative inverse gamma prior, with a shape of 1.5 and a rate of 20.
...
**Exercise 6**
Set up a weakly informative prior for the continuous emission distribution using the function `prior_emiss_cont()` and save the object in `emotion_prior_emiss`. Keep the above considerations in mind. We already provide the structure of the object and you only have to specify the priors as indicated above:
...
We are now ready to fit a 2-state multilevel Hidden Markov Model using the function `mHMM()`. For now, set the number of iterations `J` as part of the input parameter `mcmc` to 500, and specify a burn in (`burn_in`) of 200. On a reasonably fast laptop, this should take about 10 minutes. In the meantime, you can have a nice conversation with your neighbor or workshop instructor :-). When fitting a multilevel hidden Markov model for research purposes, we recommend using at least 2.000 iterations, and using traceplots and the Gelman Rubin statistic to ensure convergence with the used number of iterations and the appropriate burn in period. We will discuss convergence in more detail in the second practical of this workshop.

---

**Exercise 7**
Fit a 2-state multilevel Hidden Markov Model using the function `mHMM()` and save your fitted model in an object named `out_2st_emotion`. See below for the general structure of the `mHMM()` function. The provided code assumes continuous observations, no level 2 covariates and only a prior on the Normal emission distribution.

The function `mHMM()` takes the following arguments that are relevant for now:

* s_data: the dataset to build the model for. The first column should contain the subject IDs, and the subsequent columns should contain the modeled (dependent) variables.
* data_distr: the distribution of the dependent variables. Here we model variables as Gaussian distributions and choose "continuous" accordingly; but also "categorical" and "count" are options.
...
* mcmc: a list with two entries, containing the number of iterations `J` and the burn in period `burn_in`.
...
Especially when you have a large number of states or/and a large number of dependent variables, visualizing the group level parameters can be very helpful. 
...
To inspect the obtained most likely state sequences, plotting them is most useful. State sequences can be plotted per subject and optionally together with the observed time series data. The *ggmHMM* package contains a function `plot_viterbi()`, which can be used to plot a barcode plot of the most likely state sequences over time. It takes the following arguments:
...
In this first practical, we have loaded and inspected our data, fitted a 2-state multilevel hidden Markov model, investigated and visualized our group level parameters, and obtained (and visualized) the most likely sequence of hidden states. 

In the next practical, we will focus on the model selection problem (how many states should we use?), obtaining and interpreting subject-specific parameters, and model checking (inspecting convergence and label switching, and using posterior predictive checks and pseudoresiduals to evaluate different aspects of model fit). If you finished early with this part, feel free to already start on the second part.
